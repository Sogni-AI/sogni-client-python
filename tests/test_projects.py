...
async def test_failed_submission_disposes_unsubmitted_project(
    ...
    initialize = Project.__init__

    def capture_project(self: Project, *args: Any, **kwargs: Any) -> None:
        initialize(self, *args, **kwargs)
        captured.append(self)
        assert self._timeout_handle is not None
        timer_handles.append(self._timeout_handle)

    monkeypatch.setattr(Project, "__init__", capture_project)
    api.get_model_options = AsyncMock(return_value=model_options("video"))  # type: ignore[method-assign]
    api._process_assets = AsyncMock()  # type: ignore[method-assign]
    client.socket.send = AsyncMock()  # type: ignore[method-assign]
    failure = RuntimeError("submission failed")
    if phase == "options":
        api.get_model_options.side_effect = failure
    elif phase == "assets":
        api._process_assets.side_effect = failure
    elif phase in {"send", "cancel"}:

        async def fail_send(*args: Any) -> None:
            assert captured[0].id in api._unadmitted_requests
            if phase == "cancel":
                raise asyncio.CancelledError
            raise failure

        client.socket.send.side_effect = fail_send

    params = _h3_params("minimax-h3-fastvideo-int8_t2v_turbo", 20 if phase == "validation" else 4)
    expected_error = (
    ...
    assert timer_handles[0].cancelled()
    assert captured[0]._timeout_handle is None
    assert captured[0]._completion.cancelled()
    assert api._projects == []
    assert api._unadmitted_requests == {}
